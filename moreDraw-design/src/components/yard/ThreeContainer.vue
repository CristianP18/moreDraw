    ...
    <div class="three-container">
        ...
        >
            ...
            <div class="tool-buttons">
                ...
                <!-- Botão para ativar TransformControls -->
                <q-btn
                    round
                    color="secondary"
                    icon="transform"
                    class="tool-btn"
                    @click="toggleTransformControls"
                >
                    <q-tooltip>Ativar Transformações</q-tooltip>
                </q-btn>
            </div>
        </div>
        ...
        >
            ...
            <!-- Botões para selecionar o modo de transformação -->
            <div class="transform-mode-buttons">
                <q-btn
                    ...
                    color="blue"
                    label="Mover"
                    @click="changeTransformMode('translate')"
                />
                <q-btn
                    flat
                    ...
                    color="blue"
                    label="Rotacionar"
                    @click="changeTransformMode('rotate')"
                />
                <q-btn
                    flat
                    ...
                    color="blue"
                    label="Escalar"
                    @click="changeTransformMode('scale')"
                />
            </div>

            <!-- Seção de Inputs de Rotação -->
            <div
            ...
            >
                <q-input
                    ...
                    suffix="°"
                    debounce="300"
                    @input="applyRotation('x', rotation.x)"
                />
                <q-input
                    ...
                    suffix="°"
                    debounce="300"
                    @input="applyRotation('y', rotation.y)"
                />
                <q-input
                    ...
                    suffix="°"
                    debounce="300"
                    @input="applyRotation('z', rotation.z)"
                />
            </div>
        </div>
    </div>
...
import { TransformControls } from 'three/examples/jsm/controls/TransformControls.js'; // Importar TransformControls
...
let renderer;
let orbitControls;
let transformControls; // Variável para TransformControls
let model;
...
// Estado para controlar o TransformControls
const isTransformMode = ref(false);
const transformMode = ref('translate'); // Pode ser 'translate', 'rotate', 'scale'

// Estado para os inputs de rotação
...
watch(
    ...
    (newVal) => {
        if (orbitControls) {
            orbitControls.enabled = !isTransformMode.value && newVal;
            Notify.create({
...
function initThree() {
    ...
    orbitControls.enableZoom = true; // Ativa o zoom com a roda do mouse
    orbitControls.zoomSpeed = 1.0; // Ajuste a velocidade do zoom conforme necessário
    orbitControls.enabled = props.showTools; // Controlado pela prop
    ...
    // Configuração do TransformControls
    transformControls = new TransformControls(camera, renderer.domElement);
    transformControls.addEventListener('dragging-changed', (event) => {
        orbitControls.enabled = !event.value;
        isTransformMode.value = event.value;
    });
    transformControls.addEventListener('objectChange', sendObjectPosition); // Atualizar posição após mudança
        ...
        case 'glb':
            ...
            loader.load(
                ...
                (gltf) => {
                    ...
                    scene.add(model);
                    centerModel(model);
                    transformControls.attach(model); // Anexar TransformControls ao modelo
...
function centerModel(model) {
    ...
    // Atualizar controles
    orbitControls.maxDistance = cameraZ * 10;
...
function sendObjectPosition() {
    if (model) {
        ...
        // Atualizar os inputs de rotação para refletir a rotação atual do modelo
        rotation.value.x = THREE.MathUtils.radToDeg(model.rotation.x).toFixed(2);
        rotation.value.y = THREE.MathUtils.radToDeg(model.rotation.y).toFixed(2);
        rotation.value.z = THREE.MathUtils.radToDeg(model.rotation.z).toFixed(2);
    } else {
...
// Função para aplicar rotação com base nos inputs
function applyRotation(axis, value) {
    if (model) {
        const rad = THREE.MathUtils.degToRad(value);
        if (axis === 'x') model.rotation.x = rad;
        if (axis === 'y') model.rotation.y = rad;
        if (axis === 'z') model.rotation.z = rad;

        console.log(`Rotação aplicada no eixo ${axis}: ${value} graus`);
        sendObjectPosition();
    }
}
...
function toggleControlPanel() {
    ...
    if (showControlPanel.value && model) {
        transformControls.attach(model); // Anexar ao modelo quando o painel é aberto
    } else {
        transformControls.detach(); // Desanexar quando o painel é fechado
    }
}

// Função para alternar o modo de transformação (translate, rotate, scale)
function changeTransformMode(mode) {
    transformMode.value = mode;
    transformControls.setMode(mode);
    Notify.create({ message: `Modo de transformação: ${mode}`, color: 'info' });
}

// Função para alternar o TransformControls via botão
function toggleTransformControls() {
    if (isTransformMode.value) {
        transformControls.detach();
        orbitControls.enabled = props.showTools;
        isTransformMode.value = false;
        Notify.create({ message: 'Transformações desativadas', color: 'negative' });
    } else {
        if (model) {
            transformControls.attach(model);
            isTransformMode.value = true;
            orbitControls.enabled = false;
            Notify.create({ message: 'Transformações ativadas', color: 'positive' });
        } else {
            Notify.create({ message: 'Nenhum modelo 3D carregado para transformar.', color: 'warning' });
        }
    }
}
</script>
...
.tools-panel {
    ...
    gap: 10px;
    background: rgba(255, 255, 255, 0.3); /* Fundo mais transparente */
    padding: 10px;
    border-radius: 8px;
}

/* Container dos Botões de Ferramentas */
.tool-buttons {
    display: flex;
    flex-wrap: wrap;
    gap: 10px;
...
.control-panel {
    position: absolute;
    top: 100px;
    left: 260px; /* Posicionado à direita do tools-panel */
    z-index: 10;
    width: 300px;
...
.transform-mode-buttons q-btn {
    flex: 1;
}

/* Estilos para os inputs de rotação */
.rotation-controls {
    display: flex;
    flex-direction: column;
    gap: 10px;
