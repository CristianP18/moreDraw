<template>
    ...
    >
        ...
        >
            ...
            >
                ...
                <line
                    ...
                    :fill="item.backgroundImage ? 'url(#bg-pattern-' + item.id + ')' : 'none'"
                />
        ...
        <!-- =======================================================================
         ...
         ======================================================================= -->
        <div
            v-else-if="isGeometricFigureExtended"
            class="svg-wrapper"
        >
            <svg
                ref="svgRef"
            ...
            >
                ...
                <g v-else-if="item.type === 'container'">
                    <!-- Supondo que 'container' seja uma figura geométrica específica.
                 Ajuste conforme a necessidade específica do 'container'. -->
                    <rect
                        :width="item.width"
                        :height="item.height"
                        :fill="
                ...
                <g v-else>
                    <polygon
                        :points="getPolygonPoints(item.type, item.width, item.height) || '0,0'"
                        :fill="
                            item.backgroundImage
                                ? 'url(#bg-pattern-' + item.id + ')'
                                : (item.color || 'transparent')
        ...
        <div
            ...
            :style="{
                ...
                width: '300px', // Definido para consistência
                // height pode ser automático ou definido conforme necessidade
            }"
            ...
            <div class="card-body">
                ...
                >
                    <button
                        ...
                        @click="toggleCurvesInputs"
                    >
                        {{ showCurvesInputs ? 'Esconder Controladores' : 'Mostrar Controladores' }}
                    </button>
...
const emits = defineEmits(['select-item', 'update-item', 'bring-to-front', 'send-to-back']);
...
const config = ref({
    ...
    backgroundImage: props.item.backgroundImage || '', // Adicionado
});
...
const isGeometricFigureExtended = computed(() => {
    const extendedTypes = [
        ...
        'rhombus', 'parallelogram', 'trapezoid', 'cross',
        'chevronLeft', 'chevronRight', 'chevronUp', 'chevronDown',
        'halfStar', 'arc', 'heart', 'plus', 'moon', 'gear',
        'waveShape', 'archimedeanSpiral', 'zigzag', 'cube',
        'cylinder', 'sphere', 'cone', 'torus', 'pyramid',
        'prism', 'helix', 'spiral', 'crescent', 'flower',
        'sun', 'cloud', 'bolt',
        'rectangle', // Adicionado
        'container', // Adicionado
        'ellipse', // Adicionado
    // IMPORTANTE: 'squarePoliline' e 'circlePoliline' NÃO entram aqui
    ];
...
const svgWidth = computed(() => {
    ...
    if (props.item.type === 'bezier' && props.item.segments) {
        const allX = props.item.segments.flatMap((seg) => [seg.x, seg.cx1, seg.cx2]);
...
const svgHeight = computed(() => {
    ...
    // Bezier
    if (props.item.type === 'bezier' && props.item.segments) {
        const allY = props.item.segments.flatMap((seg) => [seg.y, seg.cy1, seg.cy2]);
...
function getPolygonPoints(type, width, height) {
    ...
    switch (type) {
        case 'star': {
            ...
            points = createStarPoints(centerX, centerY, outerRadius, innerRadius, sides);
            break;
        }
        ...
        case 'halfStar': {
            ...
            const innerRadius = radius / 2;
            // fillFactor de 0.5
            points = createStarPoints(centerX, centerY, outerRadius, innerRadius, sides, 0.5);
            break;
        }
        ...
        case 'parallelogram': {
            ...
            points = `${skewOffset},0 ${width},0 ${width - skewOffset},${height} 0,${height}`;
            break;
        }

        case 'trapezoid': {
            points = `${width * 0.25},0 ${width * 0.75},0 ${width},${height} 0,${height}`;
            break;
        }
      ...
      `;
        ...
        case 'arrowRight': {
            ...
            switch (type) {
                case 'arrowUp':
                    arrowPoints = `${arrowWidth / 2},0 ${arrowWidth},${arrowHeight} 0,${arrowHeight}`;
                    break;
                case 'arrowDown':
          ...
          `;
                    break;
                default:
                    console.warn(`getPolygonPoints: Tipo de polígono não reconhecido: ${type}`);
                    return '';
            }
...
const itemStyle = computed(() => {
    const x = (props.item.position.x + (props.panOffset?.x || 0)) * (props.zoomLevel || 1);
    const y = (props.item.position.y + (props.panOffset?.y || 0)) * (props.zoomLevel || 1);
...
function onPolylineDrag(e) {
    ...
    if (draggingIndex.value >= 0 && draggingIndex.value < props.item.path.length) {
        props.item.path[draggingIndex.value].x = x;
        props.item.path[draggingIndex.value].y = y;
...
function updateControlPoints(newCount) {
    ...
    if (props.item.type === 'bezier') {
        if (!props.item.segments) {
            props.item.segments = [];
        }
        while (props.item.segments.length < count) {
            props.item.segments.push({
                x: 50,
                y: 50,
                cx1: 30,
                cy1: 30,
        ...
        while (
            props.item.segments.length > count
      && props.item.segments.length > 1
        ) {
            props.item.segments.pop();
        }
    } else {
    // polylines, squarePoliline, circlePoliline
        if (!props.item.path) {
            props.item.path = [];
        }
        while (props.item.path.length < count) {
            props.item.path.push({ x: 0, y: 0 });
        }
        while (
            props.item.path.length > count
      && props.item.path.length > 2
        ) {
            props.item.path.pop();
        }
    }
...
.property-group input[type='text'] { /* Adicionado 'text' para backgroundImage */
  padding: 5px;
  font-size: 14px;
}
