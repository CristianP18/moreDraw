<template>
  ...
  >
    ...
    />
    <!-- Triângulo -->
    <div v-else-if="item.type === 'triangle'" class="triangle-shape">
      <svg
        :width="item.width"
        :height="item.height"
    ...
    <div v-else-if="item.type === 'line'" class="line-item">
      <svg :width="itemWidth" :height="itemHeight" :style="svgStyle">
        <line
          :x1="0"
          :y1="0"
          :x2="lineDeltaX"
...
const itemWidth = computed(
  () =>
    Math.abs(props.item.endPosition.x - props.item.position.x) * props.zoomLevel
);

const itemHeight = computed(
  () =>
    Math.abs(props.item.endPosition.y - props.item.position.y) * props.zoomLevel
);

const lineDeltaX = computed(
  () => (props.item.endPosition.x - props.item.position.x) * props.zoomLevel
);

const lineDeltaY = computed(
  () => (props.item.endPosition.y - props.item.position.y) * props.zoomLevel
);
