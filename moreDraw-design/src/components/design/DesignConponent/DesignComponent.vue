<template>
  ...
  <q-page class="page q-px-lg">
    ...
    <q-dialog v-model="isColorPickerOpen" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          />
          <q-btn flat label="Confirmar" color="primary" @click="confirmColor" />
        </q-card-actions>
      </q-card>
    </q-dialog>

    <!-- Modal de Crop Image (com pré-visualização opcional) -->
    <q-dialog v-model="showCropModal" persistent maximized>
      <q-card class="crop-modal">
        ...
        <q-card-actions align="right">
          <q-btn
            ...
            @click="cancelCrop"
          />
          <q-btn flat label="Salvar" color="primary" @click="saveCropImage" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
watch(selectedItem, (newVal) => {
  if (newVal) {
    if (newVal.type === "polyline") {
      console.log(
        'watch(selectedItem): Item é polyline, definindo controllerMode="none"'
      );
...
watch(totalImages, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    ...
    console.log(
      `watch(totalImages): totalImages mudou de ${oldVal} para ${newVal}. imagesLoaded resetado para 0.`
    );
  }
});
...
onMounted(() => {
  ...
  console.log(
    `onMounted: selectedProjectName set to "${selectedProjectName.value}"`
  );
...
function updateWindowSize() {
  ...
  console.log(
    `updateWindowSize: windowWidth=${windowWidth.value}, windowHeight=${windowHeight.value}`
  );
}
...
function addItem(newItem) {
  console.log(
    "addItem: Recebido newItem emitido por DrawingAreaComponent:",
    newItem
  );
  items.push(newItem);
  selectedItem.value = newItem;
...
function updateItems(updatedItem) {
  console.log(
    "updateItems: Recebido updatedItem emitido por DrawingAreaComponent:",
    updatedItem
  );
...
function updateItem(updatedItem) {
  console.log(
    "updateItem: Recebido updatedItem emitido por ControlPanelComponent:",
    updatedItem
  );
  const index = items.findIndex((i) => i.id === updatedItem.id);
  if (index !== -1) {
...
function toggleControllers() {
  ...
  console.log(
    `toggleControllers: showControllers is now ${showControllers.value}`
  );
}
...
function updateControllerMode(mode) {
  ...
  if (selectedItem.value) {
    ...
    console.log(
      `updateControllerMode: Updated selectedItem (${selectedItem.value.id}) controllerMode to "${mode}"`
    );
    saveState();
  }
}
...
function updateControllerCount(newCount) {
  ...
  if (selectedItem.value) {
    ...
    console.log(
      `updateControllerCount: Updated selectedItem (${selectedItem.value.id}) controllerCount to ${newCount}`
    );
    saveState();
  }
}
...
function updateRoundedLine(isRounded) {
  if (selectedItem.value) {
    ...
    console.log(
      `updateRoundedLine: Updated selectedItem (${selectedItem.value.id}) roundedLine to ${isRounded}`
    );
    saveState();
  }
}
...
function updateItemControlPoints(newCount) {
  if (!selectedItem.value || selectedItem.value.type !== "polyline") {
    console.log(
      "updateItemControlPoints: Nenhum item selecionado ou item não é polyline."
    );
    return;
  }
  ...
  if (newCount > currentCount) {
    ...
    console.log(
      `updateItemControlPoints: Adicionados ${additionalPoints} pontos ao path.`
    );
  } else if (newCount < currentCount) {
    const removedPoints = currentCount - newCount;
    currentPath.splice(newCount);
    console.log(
      `updateItemControlPoints: Removidos ${removedPoints} pontos do path.`
    );
  }
  ...
  saveState();
  console.log(
    "updateItemControlPoints: Item atualizado com novos pontos de controle:",
    currentPath
  );
}
...
function handleImageUploadInItemBackground(imageUrl) {
  console.log(
    `handleImageUploadInItemBackground: Recebido imageUrl=${imageUrl}`
  );
  if (selectedItem.value) {
    ...
    img.onload = () => {
      ...
      console.log(
        `Background da imagem definido para o item ID=${selectedItem.value.id}`
      );
...
function add3DModelToCanvas(modelUrl) {
  ...
  if (extension === "obj") {
    ...
    console.error(
      `add3DModelToCanvas: Formato de arquivo não suportado: ${extension}`
    );
    return;
  }
  ...
  loader.load(
    ...
    (object) => {
      ...
      selectedItem.value = newItem;
      console.log(
        `add3DModelToCanvas: Modelo 3D adicionado com ID=${newItem.id}`
      );
    ...
    (xhr) => {
      console.log(
        `add3DModelToCanvas: ${((xhr.loaded / xhr.total) * 100).toFixed(
          2
        )}% carregado`
      );
    },
...
function handleBringToFront(item) {
  console.log(
    `handleBringToFront: Trazendo para frente o item com ID=${item.id}`
  );
  ...
  if (index !== -1) {
    ...
    console.log(
      `handleBringToFront: Item com ID=${item.id} trazido para frente`
    );
    saveState();
  } else {
...
function saveState() {
  ...
  if (changeCounter >= 30) {
    if (history.past.length >= 50) {
      history.past.shift();
      console.log(
        "saveState: Histórico past excedeu 50, removendo o mais antigo"
      );
    }
...
function handleKeyDown(event) {
  ...
  if (event.key === "Delete" && selectedItem.value) {
    console.log(
      `handleKeyDown: Removendo item com ID=${selectedItem.value.id}`
    );
    ...
    if (selectedItem.value) {
      ...
      console.log(
        `handleKeyDown: Item copiado com ID=${selectedItem.value.id}`
      );
    }
...
function saveCropImage() {
  ...
  html2canvas(element, {
    ...
    .then((canvas) => {
      ...
      link.click();
      console.log(
        'saveCropImage: CropImage capturada e salva como "cropImage.png"'
      );
      showCropModal.value = false;
      cropPreviewSrc.value = "";
    })
...
function validateFileSize(file) {
  ...
  if (file.size > maxSizeMB * 1024 * 1024) {
    $q.notify({
      message: `Tamanho máximo de ${maxSizeMB}MB excedido.`,
      color: "negative",
    });
    console.warn(
      `validateFileSize: Arquivo muito grande (${file.size} bytes).`
    );
    return false;
  }
  return true;
...
async function updateProject() {
  ...
  if (!createForm.value || !createForm.value.imageId) {
    ...
    $q.notify({
      message: "Nenhum projeto selecionado para atualizar.",
      color: "warning",
    });
    return;
  }
  ...
  if (projectData.items) {
    try {
      ...
      const parsedItems = JSON.parse(
        JSON.stringify(JSON.parse(projectData.items))
      );
      items.splice(0, items.length, ...parsedItems);
      ...
      if (projectData.description) {
        ...
        console.log(
          `openProject: description set to "${projectData.description}"`
        );
      }
      ...
      console.log(
        `openProject: selectedProjectName set to "${selectedProjectName.value}"`
      );
    ...
    $q.notify({
      message: "Nenhum item encontrado no projeto.",
      color: "warning",
    });
...
async function loadImages() {
  ...
  console.log(
    "loadImages: Função para carregar imagens/projetos não implementada."
  );
}
...
function handleImagesLoaded() {
  imagesLoaded.value++;
  console.log(
    `handleImagesLoaded: Imagem carregada. Total carregadas: ${imagesLoaded.value}/${totalImages.value}`
  );
}
