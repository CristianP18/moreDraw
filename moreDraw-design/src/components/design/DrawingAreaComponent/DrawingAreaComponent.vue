...
// Observa mudança na URL de background
watch(
      ...
      if (newUrl !== oldUrl) {
          ...
          if (selectedItemBackground.value) {
              if (supportedBackgroundTypes.includes(selectedItemBackground.value.type)) {
                  // Cópia para não mutar diretamente
                  const updatedItem = {
                      ...selectedItemBackground.value,
                      backgroundImage: newUrl,
                  };
                  emits('update-items', updatedItem);
                  console.log('Background da imagem atualizado no item ID=', updatedItem.id);
              } else {
...
function advancedPaintFill(x, y) {
  ...
  if (clickedItem) {
      // Em vez de mutar clickedItem.color = props.selectedColor,
      // fazemos uma cópia do objeto
      const updated = { ...clickedItem, color: props.selectedColor };
      emits('update-items', updated);
      emits('save-state');
...
function handleMouseDown(e) {
  ...
  if (props.paintMode) {
      if (props.paintType === 'paint') {
          ...
          if (clicked) {
              // Criar cópia em vez de mutar
              const updatedItem = { ...clicked, color: props.selectedColor };
              emits('update-items', updatedItem);
              emits('save-state');
          }
      }
          ...
          switch (it.type) {
              case 'line': {
                  ...
                  updated = {
                      ...
                      endPosition: {
                          x: it.endPosition.x + dx,
                          y: it.endPosition.y + dy,
                      },
                  };
                  break;
              }
              ...
              case 'bezier': {
                  // Mover path inteiro
                  const newPath = it.path.map((pt) => ({
                      x: pt.x + dx,
                      y: pt.y + dy,
                  }));
                  updated = { ...it, path: newPath };
                  break;
              }
              case 'image':
              case '3dModel':
...
function updateSquarePoliline(newCount) {
  ...
  const pathCopy = [...props.selectedItem.path];
  const totalPoints = pathCopy.length;
  const adjustedCount = Math.max(4, newCount);

  if (adjustedCount > totalPoints) {
      for (let i = totalPoints; i < adjustedCount; i++) {
          const last = pathCopy[pathCopy.length - 1] || { x: 0, y: 0 };
          pathCopy.push({ x: last.x, y: last.y });
      }
  } else if (adjustedCount < totalPoints) {
      pathCopy.splice(adjustedCount, totalPoints - adjustedCount);
  }
  pathCopy[pathCopy.length - 1] = { ...pathCopy[0] };
...
function updateCirclePoliline(newCount) {
  ...
  const pathCopy = [...props.selectedItem.path];
  const currentCount = pathCopy.length;
  ...
  if (newCount > currentCount) {
      ...
      pathCopy.splice(newCount, currentCount - newCount);
  }
  pathCopy[pathCopy.length - 1] = { x: pathCopy[0].x, y: pathCopy[0].y };
...
function toggleRoundedLine() {
  if (props.selectedItem) {
      const updated = { ...props.selectedItem, roundedLine: !props.selectedItem.roundedLine };
      emits('update-items', updated);
  }
}
