<template>
    <div class="q-pa-md container-imagem-yard">
        ...
        <div class="botoes-acoes">
            ...
            <q-btn
                color="secondary"
                :label="$t('imageYard.manageImages')"
        ...
        <q-dialog v-model="dialogCreateVisible">
            <q-card class="modal-card">
                ...
                <q-card-section>
                    <q-form @submit.prevent="createImageYard">
                        <q-input
                            v-model="createForm.imageName"
                            :label="$t('imageYard.imageName')"
                            outlined
                            required
                            class="q-mb-md"
                            :disable="loadingCreate"
                        ...
                        <q-card-actions align="right">
                            <q-btn
                                flat
                                :label="$t('imageYard.cancel')"
                                :disable="loadingCreate"
                                @click="dialogCreateVisible = false"
                            />
                            <q-btn
                                color="primary"
                                :label="$t('imageYard.register')"
                                type="submit"
                                unelevated
                                :loading="loadingCreate"
                                :disable="loadingCreate"
        ...
        <q-dialog
            v-model="dialogManageVisible"
            persistent
        >
            <q-card style="min-width: 500px; min-height: 300px;">
                <q-card-section class="row items-center justify-between modal-header">
                    <div class="text-h6">
                        {{ $t('imageYard.manageTitle') }}
                    </div>
                    <q-btn
                        flat
                        icon="close"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <q-btn
                                    v-if="imageToEditImage && imageToEditImage.imageId === img.imageId"
                                    size="sm"
                                    flat
                                    icon="edit"
                                    color="primary"
                                    style="position: absolute; top: 5px; right: 5px; background: rgba(255,255,255,0.8); border-radius:50%;"
                                    :loading="loadingUpdateImage && imageToEditImage.imageId === img.imageId"
                                    :disable="loadingUpdateImage && imageToEditImage.imageId === img.imageId"
                            ...
                            <div class="row justify-between q-mt-xs w-100">
                                ...
                                <q-btn
                                    ...
                                    size="sm"
                                    color="negative"
                                    :loading="loadingDelete && imageToDelete?.imageId === img.imageId"
        ...
        <q-dialog v-model="dialogEditVisible">
            <q-card class="modal-card">
                <q-card-section class="row items-center justify-between modal-header">
                    <div class="text-h6">
                        {{ $t('imageYard.editAttributes') }} - {{ editForm.imageName }}
                    </div>
                    <q-btn
                        flat
                        icon="close"
                ...
                <q-card-section>
                    <q-form @submit.prevent="updateImageYard">
                        <q-input
        ...
        <q-dialog v-model="dialogEditImageVisible">
            <q-card class="modal-card">
                <q-card-section class="row items-center justify-between modal-header">
                    <div class="text-h6">
                        {{ $t('imageYard.editImage') }} - {{ imageToEditImage ? imageToEditImage.imageName : '' }}
                    </div>
                    <q-btn
                        flat
                        icon="close"
                ...
                <q-card-section>
                    <q-form @submit.prevent="updateImageYardImage">
                        <q-file
                            v-model="editImageFiles"
                            :label="$t('imageYard.selectNewImage')"
                            filled
                            bottom-slots
                            :counter="true"
                            accept="image/*"
        ...
        <q-dialog v-model="dialogDeleteVisible">
            <q-card>
                ...
                <q-card-section>
                    <div>
                        {{ $t('imageYard.deleteConfirmation', { imageName: imageToDelete ? imageToDelete.imageName : '' }) }}
                    </div>
                </q-card-section>
...
// Criar ImageYard (POST com form-data)
const createImageYard = async () => {
    loadingCreate.value = true;

    const file = Array.isArray(createSelectedFile.value) ? createSelectedFile.value[0] : createSelectedFile.value;
    ...
    fd.append('file', file);
    fd.append('jsonImageYard', JSON.stringify({
    ...
    try {
        ...
        console.error('Erro ao criar ImageYard:', error.response ? error.response.data : error);
...
// Atualizar atributos do ImageYard (PUT)
const updateImageYard = async () => {
    ...
    try {
        ...
        $q.notify({ message: 'Atributos atualizados com sucesso!', color: 'positive' });
        dialogEditVisible.value = false;
        await loadImages();
    } catch (error) {
        console.error('Erro ao atualizar atributos:', error);
        $q.notify({ message: 'Erro ao atualizar atributos do ImageYard.', color: 'negative' });
    }
...
const updateImageYardImage = async () => {
    loadingUpdateImage.value = true;

    if (!imageToEditImage.value) {
        $q.notify({ message: 'Nenhuma imagem selecionada para edição.', color: 'warning' });
        loadingUpdateImage.value = false;
        return;
    }

    const file = editImageFiles.value && Array.isArray(editImageFiles.value) ? editImageFiles.value[0] : editImageFiles.value;

    if (!file) {
        $q.notify({ message: 'Selecione uma imagem para atualizar.', color: 'warning' });
        loadingUpdateImage.value = false;
        return;
    }
    ...
    try {
        ...
        $q.notify({ message: 'Erro ao atualizar imagem do ImageYard.', color: 'negative' });
    }
    loadingUpdateImage.value = false;
};
